"""
Это главный файл обучения. Он запускает полный цикл:
— загрузка датасета;
— создание модели;
...
# Добавляет tqdm-прогрессбар, если он включён и установлен
...
# Фиксирует случайные генераторы для повторяемости эксперимента
...
# Выбирает устройство обучения: cuda при наличии или cpu
...
# Создаёт autocast-контекст для mixed precision на CUDA
...
# Создаёт GradScaler для устойчивого обучения в смешанной точности
...
# Совместимая загрузка checkpoint для разных версий PyTorch
...
# Инициализирует параметры Focal Loss: gamma, alpha и способ усреднения
...
# Создаёт CrossEntropyLoss или FocalLoss с заданным alpha
...
# Создаёт DataLoader для train/val/test с нужным shuffle и pin_memory
...
# Считает количество правильных предсказаний в batch
...
# Выполняет одну эпоху обучения: forward, loss, backward, optimizer step, accuracy
def train_one_epoch(model, loader, criterion, optimizer, scaler, device, amp: bool, grad_clip: float,
                    show_progress: bool = False, epoch: int = 0):
	model.train()
	loss_sum = 0.0
	correct_sum = 0
	n_sum = 0
	iterator = make_progress(loader, show_progress, desc="train epoch {}".format(epoch) if epoch else "train",
	                         total=len(loader))
...
# Оценивает модель без градиентов и собирает loss, accuracy, метки, предсказания и пути
@torch.no_grad()
...
# Рассчитывает precision, recall и F1 для каждого класса по матрице ошибок
...
# Сохраняет историю обучения по эпохам в CSV
def save_history(path: Path, rows: Sequence[Dict]) -> None:
	write_dict_csv(path, rows)


# Строит нормированную по строкам матрицу ошибок и сохраняет её как изображение
...
# Сохраняет график изменения train_acc и val_acc по эпохам
...
# Группирует предсказания по ОСШ и считает accuracy для каждого уровня
def per_snr_accuracy(paths: Sequence[str], targets: np.ndarray, preds: np.ndarray, split_root: Path,
                     snr_map: Dict[str, float]) -> List[Dict]:
	buckets = {}
...
# Сохраняет метрики, матрицу ошибок, per-class отчёт и per-SNR accuracy
def save_reports(result: Dict, out_dir: Path, split_root: Path, snr_map: Dict[str, float], class_names: Sequence[str],
                 prefix: str) -> Dict:
...
# Создаёт Adam или AdamW для оптимизации параметров модели
def build_optimizer(args: argparse.Namespace, model: nn.Module):
	if args.optimizer == "adamw":
		return torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
	return torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

# Преобразует строку class-order в список классов или включает auto-режим
def parse_class_order(value: str) -> Optional[List[str]]:
	if value.strip().lower() == "auto":
		return None
	return [item.strip() for item in value.split(",") if item.strip()]

# Описывает и считывает параметры запуска обучения из командной строки
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument("--class-order", default=",".join(DEFAULT_CLASSES),
	                    help="Comma-separated class order, or 'auto'")
	return parser.parse_args()

# Главный сценарий обучения: данные -> модель -> обучение -> валидация -> отчёты -> итоговая оценка
def main() -> None:
	args = parse_args()
	set_seed(args.seed)
	...
	test_loader = make_loader(test_ds, args.batch_size, args.workers, train=False,
	                          device=device) if test_ds is not None else None
	...
	print("Samples: train={}, val={}, test={}".format(len(train_ds), len(val_ds),
	...
	for epoch in range(start_epoch, args.epochs + 1):
		t0 = time.time()
		train_loss, train_acc = train_one_epoch(model, train_loader, criterion, optimizer, scaler, device, amp,
		                                        args.grad_clip, show_progress=show_progress, epoch=epoch)
		val_result = evaluate(model, val_loader, criterion, device, amp, show_progress=show_progress,
