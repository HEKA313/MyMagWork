"""
Это главный файл обучения классических моделей
"""
...
# Служебные колонки metadata/features, которые не используются как входные признаки модели
META_COLUMNS = {"relative_path", "split", "label", "domain", "family", "snr_db", "sample_index", "fs_hz", "num_samples",
                "profile", "params_json"}


# Оборачивает цикл в tqdm, если включён прогресс и tqdm установлен
...
# Рекурсивно преобразует NumPy-объекты в JSON-совместимые типы
def to_jsonable(obj):
...
# Загружает metadata.csv и проверяет наличие обязательных колонок
...
# Формирует конфигурацию извлечения признаков на основе CLI-параметров
...
# Создаёт таблицу признаков features.csv из всех IQ-файлов или загружает её из кэша
def extract_feature_table(args: argparse.Namespace) -> pd.DataFrame:
	...
	with config_json.open("w", encoding="utf-8") as f:
		sample_fs = float(args.fs) if args.fs is not None else float(
			metadata["fs_hz"].iloc[0]) if "fs_hz" in metadata.columns else 1.0
		json.dump(asdict(build_feature_config(args, sample_fs)), f, indent=2)
	print(f"Saved features: {feature_csv}")
	return feature_df


# Возвращает список числовых колонок, которые используются как признаки модели
...
# Создаёт SVM-конвейер: заполнение пропусков, стандартизация, классификатор SVC
def make_model(model_name: str, args: argparse.Namespace, seed: int):
	...
	if model_name == "svm":
		return Pipeline([
			...
			("clf", SVC(C=args.svm_c, kernel=args.svm_kernel, gamma=args.svm_gamma, probability=args.svm_probability,
			            class_weight="balanced", random_state=seed)),
		])
	# Создаёт конвейер случайного леса с заполнением пропусков медианой
	if model_name == "random_forest":
		return Pipeline([
			("imputer", SimpleImputer(strategy="median")),
			("clf", RandomForestClassifier(n_estimators=args.rf_estimators, max_depth=args.rf_max_depth,
			                               min_samples_leaf=args.rf_min_samples_leaf, class_weight="balanced_subsample",
			                               n_jobs=args.jobs, random_state=seed)),
		])
	# Создаёт конвейер гистограммного градиентного бустинга
	if model_name in {"gradient_boosting", "hist_gradient_boosting", "hgb"}:
		return Pipeline([
			("imputer", SimpleImputer(strategy="median")),
			("clf", HistGradientBoostingClassifier(max_iter=args.gb_max_iter, learning_rate=args.gb_learning_rate,
			                                       l2_regularization=args.gb_l2, max_leaf_nodes=args.gb_max_leaf_nodes,
			                                       random_state=seed)),
		])
...
# Создаёт конвейер гистограммного градиентного бустинга
def subsample_for_svm(X: np.ndarray, y: np.ndarray, args: argparse.Namespace, seed: int) -> Tuple[
...
# Возвращает confidence: max predict_proba или softmax-подобную оценку по decision_function
...
# Сохраняет матрицу ошибок в CSV и PNG
...
# Оценивает модель на val/test, сохраняет метрики, матрицы ошибок, predictions и accuracy по ОСШ
def evaluate_split(model, X: np.ndarray, y_true: np.ndarray, labels: Sequence[str], split_df: pd.DataFrame,
                   out_prefix: Path, title: str) -> Dict[str, object]:
	...
	metrics = {
		...
		"classification_report": classification_report(y_true, y_pred, labels=np.arange(len(labels)),
		                                               target_names=list(labels), zero_division=0, output_dict=True),
	}

	save_confusion_matrix(cm, labels, out_prefix.with_suffix(".cm_counts.csv"),
	                      out_prefix.with_suffix(".cm_counts.png"), title + " counts")
	save_confusion_matrix(cm_norm, labels, out_prefix.with_suffix(".cm_norm.csv"),
	                      out_prefix.with_suffix(".cm_norm.png"), title + " normalized")
...
# Если модель поддерживает feature_importances_, сохраняет топ признаков по важности
...
# Основной конвейер обучения: признаки → цели → модели → обучение → оценка → сохранение результатов
def train_all(args: argparse.Namespace) -> None:
	...
	for target in args.targets:
		...
		for model_name in args.models:
			...
			joblib.dump({"model": model, "label_encoder": le, "feature_columns": fcols, "target": target},
			            model_dir / "model.joblib")

			model_metrics: Dict[str, object] = {"train_seconds": float(train_seconds),
			                                    "num_train_samples_used": int(len(y_fit))}
			if X_val is not None and y_val is not None and len(y_val):
				model_metrics["val"] = evaluate_split(model, X_val, y_val, labels, val_df.reset_index(drop=True),
				                                      model_dir / "val", f"{target} {canonical_name} val")
			model_metrics["test"] = evaluate_split(model, X_test, y_test, labels, test_df.reset_index(drop=True),
			                                       model_dir / "test", f"{target} {canonical_name} test")
			...
			print(
				f"Done target={target} model={canonical_name}: test_acc={model_metrics['test']['accuracy']:.4f}, test_f1={model_metrics['test']['macro_f1']:.4f}")
...
# Описывает параметры командной строки для обучения классических моделей
def build_argparser() -> argparse.ArgumentParser:
	p = argparse.ArgumentParser(description="Train classical RF classifiers on statistical and cyclic features.",
	...
	p.add_argument("--targets", nargs="+", default=["label", "domain", "family"], choices=["label", "domain", "family"],
	               help="Targets to train")
	p.add_argument("--models", nargs="+", default=["svm", "random_forest", "gradient_boosting"],
	               choices=["svm", "random_forest", "gradient_boosting", "hist_gradient_boosting", "hgb"],
	               help="Classical ML models")
...
# Точка входа: считывает CLI-аргументы и запускает обучение всех выбранных моделей
def main() -> None:
