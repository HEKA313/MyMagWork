"""
Этот файл отвечает за формирование табличного сигнального портрета. Его задача заключается в необходимости получить
комплексный IQ-сигнал и превратить его в набор числовых признаков, которые затем можно передать в классические модели
машинного обучения.
"""
...
# Малая константа для защиты от деления на ноль и log(0)
EPS = 1e-12
...
class FeatureConfig:
	...
	num_samples: int = 1024  # фиксированная длина IQ-сигнала
	welch_nperseg: int = 256  # длина сегмента для оценки спектральной плотности
	welch_nfft: int = 512  # число точек БПФ
	cyclo_enabled: bool = True  # Включение циклостационарных признаков
	cyclo_alpha_bins: int = 33  # число точек сетки по циклической частоте
	cyclo_max_alpha: float = 0.5  # максимальная нормированная циклическая частота
	cyclo_lags: Tuple[int, ...] = (0, 1, 2, 4, 8, 16, 32)  # набор задержек, по ним считаются циклостационарные признаки

# Нормирует IQ-сигнал по средней мощности, чтобы убрать влияние масштаба амплитуды
...
# Подготавливает IQ-сигнал: выравнивает форму массива, удаляет DC-смещение, меняет длину и нормирует мощность
def prepare_iq(x: NDArray[np.complex128], num_samples: int) -> NDArray[np.complex128]:
	...
	x = x - np.mean(x)
	if 0 < num_samples != x.size:
		x = resample(x, num_samples).astype(np.complex128)
	return normalize_power(x)

# Преобразует значение в float и заменяет NaN/Inf на 0.0
def safe_float(v: float) -> float:
	if not np.isfinite(v):
		return 0.0
	return float(v)

# Считает набор базовых статистик для вещественной последовательности
...
# Извлекает спектральные признаки по оценке PSD методом Уэлча
...
# Вычисляет комплексный момент M_pq = E[x^(p-q) * conj(x)^q]
...
# Преобразует комплексное число в набор вещественных признаков: real, imag, abs, angle
...
# Считает комплексные моменты и кумулянты, описывающие форму IQ-созвездия
def cumulant_features(x: NDArray[np.complex128]) -> Dict[str, float]:
	...
	for name, val in [("M20", m20), ("M21", m21), ("M40", m40), ("M41", m41), ("M42", m42), ("C20", c20), ("C21", c21),
...
# Извлекает амплитудные, энергетические, фазовые и частотные статистики IQ-сигнала
...
# Считает компактные циклостационарные признаки по сетке циклических частот и задержек
def cyclic_autocorr_summary(x: NDArray[np.complex128], cfg: FeatureConfig) -> Dict[str, float]:
	...
	# Сводит сетку циклической автокорреляции к компактным числовым признакам
...
# Главная функция: превращает IQ-сигнал в полный табличный признаковый портрет
...
# Загружает IQ-массив и частоту дискретизации из .npz-файла
...
# Описывает аргументы командной строки для извлечения признаков из одного .npz
...
# Точка входа для самостоятельного извлечения признаков из одного IQ-файла
def main() -> None:
