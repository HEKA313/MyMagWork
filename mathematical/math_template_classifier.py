...
# Параметры генерации банка эталонных сигналов
@dataclass(frozen=True)
class TemplateBankConfig:
	fs: float = 100e6  # частота дискретизации
	num_samples: int = 1024  # длина каждого эталонного IQ-сигнала
	templates_per_class: int = 16  # сколько эталонов нужно генерировать для каждого класса
	seed: int = 2026  # зерно случайности
	bw_frac_min: float = 0.05  # диапазон относительной полосы сигнала
	bw_frac_max: float = 0.30  #
	freq_offset_frac: float = 0.06  # диапазон частотного сдвига, используемый генератором
	polytime_phase_states: int = 2  # число фазовых состояний для polytime-cигналов
	include_communications: bool = False  # включать ли коммуникационные классы в эталонный банк
	include_noise_template: bool = False  # флаг под шумовой эталон


# Контейнер банка эталонов: сигналы, метки, параметры генерации и список классов
@dataclass
class TemplateBank:
...
# Коммуникационные классы со случайной информационной последовательностью; по умолчанию не включаются в банк
...
# Нормирует IQ-сигнал по L2-норме для корректного корреляционного сравнения
...
# Приводит IQ-сигнал к фиксированной длине через resample или заполнение нулями
...
# Загружает IQ из .npz, приводит к нужной длине и нормирует для классификации
...
# Определяет итоговый список классов для банка эталонов с учётом профиля и флага коммуникационных сигналов
...
# Генерирует банк нормированных эталонных IQ-сигналов для выбранных классов
...
# Считает корреляционный score между batch сигналов и всеми эталонами;
# при необходимости максимизирует score по частотному сдвигу
...
# Преобразует оценки по эталонам в оценки по классам, беря максимум внутри каждого класса
def score_batch(
		x_batch: np.ndarray,
		bank: TemplateBank,
		frequency_invariant: bool = True,
...
# Выбирает класс с максимальным score и при необходимости применяет порог для NoiseOnly
...
# Рассчитывает accuracy, macro F1 и метрики по каждому классу
...
# Сохраняет абсолютную и нормированную матрицы ошибок в CSV и PNG
def save_confusion_matrix(y_true: Sequence[str], y_pred: Sequence[str], labels: Sequence[str], out_dir: Path,
                          prefix: str) -> None:
...
# Полный сценарий оценки математического классификатора на IQ-датасете с metadata.csv
def evaluate_dataset(args: argparse.Namespace) -> None:
	...
	eval_labels = sorted(set(meta["label"].astype(str)).union(class_labels).union(
		["NoiseOnly"] if args.noise_threshold is not None else []))
...
# Описывает параметры командной строки для оценки математического классификатора
def build_argparser() -> argparse.ArgumentParser:
	...
	p.add_argument("--profile", choices=sorted(MATH_PROFILES), default="core_lpi",
	               help="Математический профиль классов")
	...
	p.add_argument("--max-samples", type=int, default=0,
	               help="Ограничить число образцов для быстрого теста, 0 = без ограничения")
	...
	p.add_argument("--include-communications", action="store_true",
	               help="Включить коммуникационные классы в банк эталонов; для случайных символов это приближенный режим")
	p.add_argument("--frequency-invariant", dest="frequency_invariant", action="store_true", default=True,
	               help="Максимизировать корреляцию по частотному сдвигу")
	p.add_argument("--no-frequency-invariant", dest="frequency_invariant", action="store_false",
	               help="Отключить максимум по частотному сдвигу")
	...
	p.add_argument("--template-chunk-size", type=int, default=32,
	               help="Сколько эталонов обрабатывать за раз при частотном поиске")
	p.add_argument("--noise-threshold", type=float, default=None,
	               help="Если max score ниже порога, предсказывать NoiseOnly")
	p.add_argument("--progress", type=int, default=1000, help="Печатать прогресс каждые N сигналов; 0 отключает")
	return p


# Точка входа: запускает оценку математического классификатора на датасете
def main() -> None:
